...
#
# Note that the XML comment above <incompatibleWith> in Mod/About/About.xml says the game "logs nothing"
# on a duplicate. The source says otherwise, and this scenario is what settles it in a running game.
#
...
#
# The packageId Coolie.Ebbbs is the one About.xml names. It has not been checked against the original,
# which is not installed on this machine: if it is wrong, the first step fails saying the mod is not loaded.
